...
export function OrderMenus({callback,data:{status,id,bookingID,orderType,row},actions,inDetail}) {
  const {addBags,addSeats,addInsurance,issueTicket,cancelOrder,pay,cancelTicket} = actions || {}
    ...
    <div>
      <div className="menuItem " onClick={() => callback && callback()}>
        ...
        <Menu
          ...
          onClick={() => issueTicket && issueTicket()}
        />
        {devStage < 1 ?
          <Menu
            value={status}
...
export function flightStatusMap(status) {
  ...
  let map = {
    'booked': 'Not Paid',
    'issuable': 'Paid',
    'pendingticketissueapproval': 'Issuing',
    'pendingticketissue': 'Issuing',
    'issued': 'Completed',
      ...
      <div className="sticky right-0 ">
        ...
        >
          <ActionContext.Consumer>
            {(value) => {
              const {bags,seats,cancel,payment,issueTicket,cancelTicket} = value || {}
              return (
