...
export default function VoidRequests({loadAll,data,close,callback}) {
    ...
    async function listQueues() {
        ...
        const res = await getVoidTicketRequests(new URLSearchParams(query)?.toString());
    ...
    async function handleDeny({_id,reason,...rest}) {
        ...
        const res = await denyVoidRequest(_id);
    ...
    const columns = [
        {field: 'orderId', headerName: 'Order Ref No'},
    ...
    return (
        <div className='flex gap-2 items-center w-full'>
            ...
            <ActionContext.Consumer>
                {
                    (value) => {
                        const {approve,deny,voidTicket,manualIssue,cancel,requestInfo} = value || {}
                        return (
                            <CustomMenu 
                                element={
                            ...
                            >
                                <div className='menuItem'>
                                    ...
                                    <MenuContainer loader={'issue'}
                                        showFor={['Approved']}
                                        onClick={() => voidTicket(params?.row?._id)} value={params.value} >Void Ticket</MenuContainer>
