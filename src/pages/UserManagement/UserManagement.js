...
export default function UserManagement() {
  ...
  const columns = [
    {field: 'status',headerName: '',
      renderCell: (params) => (
        <b className='text-theme1 font-black light-bg p-2 rounded-md'>{params.value}</b>
      )
    },
  ...
  function handleRowSelect(rows) {
    if(rows.length)
      setSelected(data.find((obj,i) => {console.log(obj); return obj.id === rows[0]}))
  }
  ...
  const Component = () => (
    ...
    <div className='flex gap-4 justify-between items-center py-4'>
      ...
      <div className='p-2 bg-secondary'>
        <CloseOutlined onClick={() => close()} className='m-1 text-2xl cursor-pointer hover:scale-110' />
      </div>
    </div>
    ...
    <div className='pt-10 flex flex-col gap-4'>
      ...
      <Button1 className='flex gap-2 !items-center !bg-red-500' onClick={setOpenRemove}>
    ...
    <div>
      ...
      <Modal1 open={openRemove} setOpen={setOpenRemove} className={''}>
        <div className='p-10 flex flex-col gap-5 items-center justify-center max-w-[400px]'>
          ...
          <p>
          This is to inform you that you’re about to delete a user called {data.name} from your data base.
          Note that is action is permanent and this user information cannot be recovered.
          </p>
          <div className='flex py-5 gap-2 w-full'>
            <Button1 className='w-auto flex-1' onClick={() => setOpenRemove(false)}>Cancel</Button1>
  ...
  return (
    <div className='p-10 flex flex-col gap-3'>
      ...
      <div className='flex gap-3 py-5'>
        <Button1 variant={'text'} className='flex-1' onClick={close}>Cancel</Button1>
